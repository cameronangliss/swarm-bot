...
def plotFits():
    ...
    if recordType == "smooth":
        ...
        bestPlot = plt.plot(xs, bestFits, color = "limegreen", linewidth = 1.2, label = "BestFits")
...
def plotLegMotions():
    colors = ["crimson", "darkorange", "limegreen", "dodgerblue", "mediumorchid", "dimgrey"]
    for i in range(0, len(legMoves)):
        ...
        plt.plot(range(101), legMovesY, color = colors[i], linestyle = "dashed", linewidth = 1.2)
...
if recordType == "default":
    legTitle = "Current RefBot's Leg Motions\nFitness = " + str(round(refFits[-1])) + " mm"
    pathTitle = "Current RefBot's Path on Floor\nFitness = " + str(round(refFits[-1])) + " mm"
elif recordType == "ref":
    legTitle = "RefBot's Leg Motions at Generation " + botGen + "\nFitness = " + str(round(refFits[-1])) + " mm"
    pathTitle = "RefBot's Path on Floor at Generation " + botGen + "\nFitness = " + str(round(refFits[-1])) + " mm"
elif recordType == "max":
    legTitle = "MaxBot's Leg Motions at Generation " + botGen + "\nFitness = " + str(round(maxFits[-1])) + " mm"
    pathTitle = "MaxBot's Path on Floor at Generation " + botGen + "\nFitness = " + str(round(maxFits[-1])) + " mm"
if recordType == "smooth":
    lists = []
